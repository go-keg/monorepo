...
type PaymentQuery struct {
	...
	withFKs      bool
	loadTotal    []func(context.Context, []*Payment) error
...
func (pq *PaymentQuery) QueryContract() *ContractQuery {
...
func (pq *PaymentQuery) WithContract(opts ...func(*ContractQuery)) *PaymentQuery {
	query := (&ContractClient{config: pq.config}).Query()
...
//		GroupBy(payment.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (pq *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	...
	if query := pq.withContract; query != nil {
		if err := pq.loadContract(ctx, query, nodes, nil,
			func(n *Payment, e *Contract) { n.Edges.Contract = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *PaymentQuery) loadContract(ctx context.Context, query *ContractQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Contract)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Payment)
	for i := range nodes {
		if nodes[i].contract_payments == nil {
			continue
		}
		fk := *nodes[i].contract_payments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "contract_payments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
