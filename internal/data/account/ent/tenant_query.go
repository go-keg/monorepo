...
func (_q *TenantQuery) QueryTenantUsers() *TenantUserQuery {
...
func (_q *TenantQuery) QueryRoles() *TenantRoleQuery {
...
func (_q *TenantQuery) WithTenantUsers(opts ...func(*TenantUserQuery)) *TenantQuery {
	query := (&TenantUserClient{config: _q.config}).Query()
...
func (_q *TenantQuery) WithRoles(opts ...func(*TenantRoleQuery)) *TenantQuery {
	query := (&TenantRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TenantQuery) WithOrganizations(opts ...func(*OrganizationQuery)) *TenantQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TenantQuery) loadTenantUsers(ctx context.Context, query *TenantUserQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantUser)) error {
...
func (_q *TenantQuery) loadRoles(ctx context.Context, query *TenantRoleQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *TenantQuery) WithNamedTenantUsers(name string, opts ...func(*TenantUserQuery)) *TenantQuery {
	query := (&TenantUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *TenantQuery) WithNamedRoles(name string, opts ...func(*TenantRoleQuery)) *TenantQuery {
	query := (&TenantRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *TenantQuery) WithNamedOrganizations(name string, opts ...func(*OrganizationQuery)) *TenantQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
