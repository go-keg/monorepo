...
func (tq *TenantQuery) QueryTenantUsers() *TenantUserQuery {
...
func (tq *TenantQuery) QueryRoles() *TenantRoleQuery {
...
func (tq *TenantQuery) WithTenantUsers(opts ...func(*TenantUserQuery)) *TenantQuery {
	query := (&TenantUserClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTenantUsers = query
	return tq
}

// WithRoles tells the query-builder to eager-load the nodes that are connected to
// the "roles" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TenantQuery) WithRoles(opts ...func(*TenantRoleQuery)) *TenantQuery {
	query := (&TenantRoleClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := tq.withTenantUsers; query != nil {
		if err := tq.loadTenantUsers(ctx, query, nodes,
			func(n *Tenant) { n.Edges.TenantUsers = []*TenantUser{} },
			func(n *Tenant, e *TenantUser) { n.Edges.TenantUsers = append(n.Edges.TenantUsers, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withRoles; query != nil {
		if err := tq.loadRoles(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Roles = []*TenantRole{} },
			func(n *Tenant, e *TenantRole) { n.Edges.Roles = append(n.Edges.Roles, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range tq.withNamedTenantUsers {
		if err := tq.loadTenantUsers(ctx, query, nodes,
			func(n *Tenant) { n.appendNamedTenantUsers(name) },
			func(n *Tenant, e *TenantUser) { n.appendNamedTenantUsers(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedRoles {
		if err := tq.loadRoles(ctx, query, nodes,
			func(n *Tenant) { n.appendNamedRoles(name) },
			func(n *Tenant, e *TenantRole) { n.appendNamedRoles(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedOrganizations {
		if err := tq.loadOrganizations(ctx, query, nodes,
			func(n *Tenant) { n.appendNamedOrganizations(name) },
			func(n *Tenant, e *Organization) { n.appendNamedOrganizations(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TenantQuery) loadTenantUsers(ctx context.Context, query *TenantUserQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantUser)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenantuser.FieldTenantID)
	}
	query.Where(predicate.TenantUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.TenantUsersColumn), fks...))
	}))
...
func (tq *TenantQuery) loadRoles(ctx context.Context, query *TenantRoleQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenantrole.FieldTenantID)
	}
	query.Where(predicate.TenantRole(func(s *sql.Selector) {
...
func (tq *TenantQuery) WithNamedTenantUsers(name string, opts ...func(*TenantUserQuery)) *TenantQuery {
	query := (&TenantUserClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedTenantUsers == nil {
		tq.withNamedTenantUsers = make(map[string]*TenantUserQuery)
	}
	tq.withNamedTenantUsers[name] = query
	return tq
}

// WithNamedRoles tells the query-builder to eager-load the nodes that are connected to the "roles"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (tq *TenantQuery) WithNamedRoles(name string, opts ...func(*TenantRoleQuery)) *TenantQuery {
	query := (&TenantRoleClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedRoles == nil {
		tq.withNamedRoles = make(map[string]*TenantRoleQuery)
	}
	tq.withNamedRoles[name] = query
	return tq
}

// WithNamedOrganizations tells the query-builder to eager-load the nodes that are connected to the "organizations"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (tq *TenantQuery) WithNamedOrganizations(name string, opts ...func(*OrganizationQuery)) *TenantQuery {
	query := (&OrganizationClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedOrganizations == nil {
		tq.withNamedOrganizations = make(map[string]*OrganizationQuery)
	}
	tq.withNamedOrganizations[name] = query
	return tq
}
