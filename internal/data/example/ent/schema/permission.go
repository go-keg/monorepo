...
func (Permission) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("key").Nillable().Optional().Unique(),
		...
		field.Bool("is_system").Default(false).Comment("是否是系统级权限"),
	}
}
...
func (Permission) Edges() []ent.Edge {
	return []ent.Edge{
		edge.From("roles", TenantRole.Type).Ref("permissions").Annotations(entgql.Skip()),
