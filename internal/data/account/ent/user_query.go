...
func (uq *UserQuery) QueryTenantUsers() *TenantUserQuery {
	query := (&TenantUserClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withTenantUsers:   uq.withTenantUsers.Clone(),
...
func (uq *UserQuery) WithTenantUsers(opts ...func(*TenantUserQuery)) *UserQuery {
	query := (&TenantUserClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withTenantUsers = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withTenantUsers; query != nil {
		if err := uq.loadTenantUsers(ctx, query, nodes,
			func(n *User) { n.Edges.TenantUsers = []*TenantUser{} },
			func(n *User, e *TenantUser) { n.Edges.TenantUsers = append(n.Edges.TenantUsers, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range uq.withNamedTenantUsers {
		if err := uq.loadTenantUsers(ctx, query, nodes,
			func(n *User) { n.appendNamedTenantUsers(name) },
			func(n *User, e *TenantUser) { n.appendNamedTenantUsers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadTenantUsers(ctx context.Context, query *TenantUserQuery, nodes []*User, init func(*User), assign func(*User, *TenantUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenantuser.FieldUserID)
	}
	query.Where(predicate.TenantUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TenantUsersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithNamedTenantUsers(name string, opts ...func(*TenantUserQuery)) *UserQuery {
	query := (&TenantUserClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if uq.withNamedTenantUsers == nil {
		uq.withNamedTenantUsers = make(map[string]*TenantUserQuery)
	}
	uq.withNamedTenantUsers[name] = query
	return uq
}
