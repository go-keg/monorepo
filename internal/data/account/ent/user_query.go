...
func (_q *UserQuery) QueryTenantUsers() *TenantUserQuery {
...
func (_q *UserQuery) WithTenantUsers(opts ...func(*TenantUserQuery)) *UserQuery {
	query := (&TenantUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	for name, query := range _q.withNamedOauthAccounts {
		if err := _q.loadOauthAccounts(ctx, query, nodes,
...
func (_q *UserQuery) loadTenantUsers(ctx context.Context, query *TenantUserQuery, nodes []*User, init func(*User), assign func(*User, *TenantUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *UserQuery) WithNamedOauthAccounts(name string, opts ...func(*OAuthAccountQuery)) *UserQuery {
	query := (&OAuthAccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedOauthAccounts == nil {
		_q.withNamedOauthAccounts = make(map[string]*OAuthAccountQuery)
	}
	_q.withNamedOauthAccounts[name] = query
	return _q
}

// WithNamedTenantUsers tells the query-builder to eager-load the nodes that are connected to the "tenant_users"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithNamedTenantUsers(name string, opts ...func(*TenantUserQuery)) *UserQuery {
	query := (&TenantUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
